...
export function IndiaHeatmap({ points }: { points: HeatmapPoint[] }) {
  ...
  if (points.length === 0) {
    return (
      <div className="flex items-center justify-center h-[400px] bg-neutral-900 rounded-lg text-neutral-400 text-sm">
  ...
  return (
    ...
    >
      ...
      {hovered && (
        <div
          className="absolute z-20 bg-neutral-900 border border-neutral-800 rounded-lg shadow-lg p-3 text-xs pointer-events-none min-w-[200px]"
          style={{
            left: Math.min(mousePos.x + 12, 500),
            top: mousePos.y + 12,
          }}
        >
          <p className="font-semibold text-neutral-100">{hovered.city}</p>
          <p className="text-neutral-500 mb-2">{hovered.state}</p>
          <div className="space-y-1">
            <div className="flex justify-between gap-4">
              <span className="text-neutral-500">Orders</span>
              <span className="font-medium text-neutral-100">{hovered.orderCount}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-neutral-500">Revenue</span>
              <span className="font-medium text-neutral-100">₹{hovered.revenue.toLocaleString()}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-neutral-500">Top product</span>
              <span className="font-medium text-neutral-100 text-right">{hovered.topProduct}</span>
            </div>
              ...
              <div className="pt-1 mt-1 border-t border-neutral-800">
                ...
                {hovered.topPincodes.map((pc) => (
                  <div key={pc.pincode} className="flex justify-between">
                    ...
                    <span className="text-neutral-100 font-medium">{pc.count}</span>
                  </div>
                ))}
              </div>
            )}
