...
function FormulaCard({
  ...
  return (
    <div className="rounded-2xl border p-6 shadow-sm" style={{ background: "#1d293d", borderColor: "#314158" }}>
      <div className="flex items-center gap-3 mb-4">
        ...
        <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#90a1b9" }}>{title}</p>
      </div>
      ...
      <div className="space-y-2">
        {variables.map((v) => (
          <div key={v.name} className="flex items-center justify-between text-xs">
            <span style={{ color: "#90a1b9" }}>{v.name}</span>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative inline-block">
          ...
          {showPicker && (
            <div className="absolute z-50 mt-2 rounded-xl border p-4 shadow-xl" style={{ background: "#1d293d", borderColor: "#314158" }}>
              {mode === "single" ? (
                <DayPicker
                  mode="single"
                ...
                <>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {PRESETS.map((p) => (
                      <button key={p.days} onClick={() => applyPreset(p.days)} className="rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-cyan-950/30" style={{ borderColor: "#314158", color: INK }}>{p.label}</button>
                    ))}
                  </div>
      ...
      {!loading && data && data.totalOrders > 0 && (
        <>
          ...
          {/* Drop-off + summary */}
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border p-5 shadow-sm" style={{ background: "#1d293d", borderColor: "#314158" }}>
              <div className="flex items-center gap-3 mb-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: `${ROSE}18`, color: ROSE }}>
                  <TrendingDown size={20} />
                </span>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#90a1b9" }}>Drop-Off Rate</p>
              ...
              <p className="mt-3 text-xs" style={{ color: "#90a1b9" }}>
            ...
            <div className="rounded-2xl border p-5 shadow-sm" style={{ background: "#1d293d", borderColor: "#314158" }}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#90a1b9" }}>The Takeaway</p>
      ...
      {!loading && data && data.totalOrders === 0 && (
        <div className="rounded-2xl border p-8 text-center" style={{ background: "#1d293d", borderColor: "#314158" }}>
          <p className="text-sm" style={{ color: "#90a1b9" }}>No orders found for this date range.</p>
        </div>
      )}
    </div>
  );
