...
function KpiCard({
  ...
  return (
    <div className="rounded-2xl border p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md" style={{ background: "#171717", borderColor: "#262626" }}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#a3a3a3" }}>{label}</p>
      ...
      {note && <p className="mt-1.5 text-xs" style={{ color: "#a3a3a3" }}>{note}</p>}
    </div>
  );
}
...
function FormulaCard({
  ...
  return (
    <div className="rounded-2xl border p-6 shadow-sm" style={{ background: "#171717", borderColor: "#262626" }}>
      <div className="flex items-center gap-3 mb-4">
        ...
        <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#a3a3a3" }}>{title}</p>
      </div>
      ...
      <div className="space-y-2">
        {variables.map((v) => (
          <div key={v.name} className="flex items-center justify-between text-xs">
            <span style={{ color: "#a3a3a3" }}>{v.name}</span>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative inline-block">
          ...
          {showPicker && (
            <div className="absolute z-50 mt-2 rounded-xl border p-4 shadow-xl" style={{ background: "#171717", borderColor: "#262626" }}>
              {mode === "single" ? (
                <DayPicker
                  mode="single"
                ...
                <>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {PRESETS.map((p) => (
                      <button key={p.days} onClick={() => applyPreset(p.days)} className="rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-amber-950/30" style={{ borderColor: "#262626", color: INK }}>{p.label}</button>
                    ))}
                  </div>
      ...
      {!loading && data && data.totalOrders > 0 && (
        <>
          ...
          {/* Drop-off + summary */}
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border p-5 shadow-sm" style={{ background: "#171717", borderColor: "#262626" }}>
              <div className="flex items-center gap-3 mb-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: `${ROSE}18`, color: ROSE }}>
                  <TrendingDown size={20} />
                </span>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#a3a3a3" }}>Drop-Off Rate</p>
              ...
              <p className="mt-3 text-xs" style={{ color: "#a3a3a3" }}>
            ...
            <div className="rounded-2xl border p-5 shadow-sm" style={{ background: "#171717", borderColor: "#262626" }}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: "#a3a3a3" }}>The Takeaway</p>
      ...
      {!loading && data && data.totalOrders === 0 && (
        <div className="rounded-2xl border p-8 text-center" style={{ background: "#171717", borderColor: "#262626" }}>
