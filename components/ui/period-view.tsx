    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative">
          <button
            ...
            className="flex items-center gap-3 px-5 py-3 bg-slate-900 border border-slate-800 rounded-xl text-sm font-medium text-slate-200 hover:bg-slate-900 transition-colors shadow-sm"
          >
          ...
          {showPicker && (
            <div className="absolute z-50 mt-2 bg-slate-900 border border-slate-800 rounded-xl shadow-lg p-3">
              <DayPicker
                mode="range"
                selected={range}
                onSelect={setRange}
              ...
              <div className="flex justify-end gap-2 px-3 pb-2">
                <button
                  ...
                  className="px-3 py-1.5 text-sm text-slate-300 hover:bg-slate-900 rounded"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="flex gap-2">
          {PRESETS.map((p) => (
            <button
              ...
              className="px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 rounded-lg border border-slate-800 bg-slate-900"
            >
              {p.label}
            </button>
          ))}
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          <div className="animate-spin h-8 w-8 border-2 border-slate-700 border-t-neutral-900 rounded-full" />
        </div>
      )}

      {!loading && !data && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-12 text-center">
          <p className="text-slate-500 text-sm">Pick a date range or preset to view period metrics.</p>
        </div>
      )}

      {!loading && data && data.totalOrders === 0 && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-8 text-center">
          <p className="text-slate-400 text-sm">No orders found in this period.</p>
        </div>
      )}
