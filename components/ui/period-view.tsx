...
export function PeriodView() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative">
          <button
            ...
            className="flex items-center gap-3 px-5 py-3 bg-[#1d293d] border border-neutral-800 rounded-xl text-sm font-medium text-neutral-300 hover:bg-[#1d293d] transition-colors shadow-sm"
          >
          ...
          {showPicker && (
            <div className="absolute z-50 mt-2 bg-[#1d293d] border border-neutral-800 rounded-xl shadow-lg p-3">
              <DayPicker
                mode="range"
                selected={range}
                onSelect={setRange}
              ...
              <div className="flex justify-end gap-2 px-3 pb-2">
                <button
                  ...
                  className="px-3 py-1.5 text-sm text-neutral-400 hover:bg-[#1d293d] rounded"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="flex gap-2">
          {PRESETS.map((p) => (
            <button
              ...
              className="px-3 py-2 text-sm text-neutral-400 hover:bg-neutral-900 rounded-lg border border-neutral-800 bg-[#1d293d]"
            >
              {p.label}
            </button>
          ))}
      ...
      {!loading && !data && (
        <div className="bg-[#1d293d] rounded-xl border border-neutral-800 p-12 text-center">
          <p className="text-neutral-400 text-sm">Pick a date range or preset to view period metrics.</p>
        </div>
      )}

      {!loading && data && data.totalOrders === 0 && (
        <div className="bg-[#1d293d] rounded-xl border border-neutral-800 p-8 text-center">
